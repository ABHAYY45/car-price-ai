...
<style>
    ...
    .result-card .result-label { font-size: 0.95rem; opacity: 0.88; margin-bottom: 0.3rem; }
    .result-card .result-price { font-size: 2.6rem; font-weight: 800; margin: 0; letter-spacing: -0.5px; }
    .result-card .result-note  { font-size: 0.82rem; opacity: 0.75; margin-top: 0.5rem; }
    ...
    div[data-testid="stMetricLabel"]  { justify-content: center; font-size: 0.95rem; color: #166534; }
    div[data-testid="stMetricValue"]  { font-size: 2.2rem; font-weight: 800; color: #15803d; }
...
with st.container(border=True):
    ...
    with col2:
        km_driven = st.number_input(
            "🛣️ Kilometres Driven",
            min_value=0, max_value=1_000_000, value=40000, step=1000,
    ...
    with col3:
        mileage = st.number_input(
            "⛽ Mileage (km/l)",
            min_value=1.0, max_value=60.0, value=18.5, step=0.5, format="%.1f",
            help="Fuel efficiency in kilometres per litre",
        )
    with col4:
        engine = st.number_input(
            "🔧 Engine (CC)",
            min_value=100, max_value=10_000, value=1197, step=50,
    ...
    with col5:
        max_power = st.number_input(
            "⚡ Max Power (bhp)",
            min_value=10.0, max_value=1000.0, value=82.0, step=1.0, format="%.1f",
...
if not is_valid:
    st.warning("⚠️ Please enter valid positive values for all numeric fields before predicting.")
...
with btn_col1:
    predict_clicked = st.button(                               # ← EDIT 2
        "🔍 Predict Price",
        type="primary",
        disabled=not is_valid,
    )
with btn_col2:
    explain_clicked = st.button(                               # ← EDIT 2
        "🧠 Explain Prediction",
        type="secondary",
        disabled=not is_valid,
    )
...
# --------------------------------------------------------------------------- #
if predict_clicked:
    if not is_valid:                                           # ← EDIT 3
        st.error("Please enter valid positive values for all fields")
    else:
        with st.spinner("Predicting price..."):                # ← EDIT 3
            try:
                ...
                with metric_col:
                    st.metric(label="Estimated Selling Price", value=format_inr(price))
                with range_col1:
                    st.metric(label="Likely Low",  value=format_inr(price * 0.88))
                with range_col2:
                    st.metric(label="Likely High", value=format_inr(price * 1.12))

                st.caption("Estimate based on current market data. Actual selling price may vary.")
...
if explain_clicked:
    if not is_valid:                                           # ← EDIT 3
        ...
        with st.spinner("Generating explanation..."):          # ← EDIT 3
            try:
                ...
                with base_col:
                    st.metric(label="Baseline (Average Prediction)", value=format_inr(base_value))
